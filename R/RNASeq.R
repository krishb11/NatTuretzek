...
pairwiseCompare <- function(BatObject) {
	if (!requireNamespace("sleuth", quietly = TRUE)) {
		stop("Package 'sleuth' is required for pairwiseCompare(). Install it from Bioconductor/r-universe.", call. = FALSE)
	}
	...
	print(s2c)
	#build sleuth object
	so <- sleuth::sleuth_prep(s2c, target_mapping = t2g, extra_bootstrap_summary = TRUE, transformation_function = function(x) log2(x + 0.5), full_model = ~conditions, read_bootstrap_tpm = TRUE)
	#test for significance
	so <- sleuth::sleuth_fit(so)
	sleuth::models(so)
	so <- sleuth::sleuth_wt(so, which_beta = paste("conditions", BatObject$species, sep=""))
	so <- sleuth::sleuth_fit(so, ~1, "reduced")
	so <- sleuth::sleuth_lrt(so, 'reduced', 'full')
	results_wt <- sleuth::sleuth_results(so, test = paste("conditions", BatObject$species, sep=""), show_all = TRUE)
	results_lrt <- sleuth::sleuth_results(so, 'reduced:full', test_type = 'lrt')
	...
	sleuth::plot_pca(so, units = "est_counts", color_by = "conditions")
	sleuth::plot_scatter(so)
	sleuth::plot_vars(so)
	dev.off()
	return(so)
}
